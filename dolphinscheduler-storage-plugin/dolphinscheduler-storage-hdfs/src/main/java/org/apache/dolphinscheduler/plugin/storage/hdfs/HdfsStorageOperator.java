...
public class HdfsStorageOperator implements Closeable, StorageOperate {
    ...
    public byte[] catFile(String hdfsFilePath) throws IOException {
        ...
        try (FSDataInputStream fsDataInputStream = getInstance().fs.open(new Path(hdfsFilePath))) {
    ...
    public List<String> catFile(String hdfsFilePath, int skipLineNums, int limit) throws IOException {
        log.info("getInstance().fs value is:{}",getInstance().fs);
        if (StringUtils.isBlank(hdfsFilePath)) {
            log.error("hdfs file path:{} is blank", hdfsFilePath);
            return Collections.emptyList();
        }

        try (FSDataInputStream in = getInstance().fs.open(new Path(hdfsFilePath))) {
    ...
    public boolean mkdir(String tenantCode, String hdfsPath) throws IOException {
        return getInstance().fs.mkdirs(new Path(addFolderSeparatorIfNotExisted(hdfsPath)));
    }
    ...
    public boolean copy(String srcPath, String dstPath, boolean deleteSource, boolean overwrite) throws IOException {
        return FileUtil.copy(getInstance().fs, new Path(srcPath), getInstance().fs, new Path(dstPath), deleteSource, overwrite, getInstance().fs.getConf());
    }
    ...
    public boolean copyLocalToHdfs(String srcFile, String dstHdfsPath, boolean deleteSource,
        ...
        getInstance().fs.copyFromLocalFile(deleteSource, overwrite, srcPath, dstPath);
    ...
    public StorageEntity getFileStatus(String path, String prefix, String tenantCode,
                                       ResourceType type) throws IOException {
        try {
            FileStatus fileStatus = getInstance().fs.getFileStatus(new Path(path));
            String alias = "";
            String fileName = "";
            String fullName = fileStatus.getPath().toString();
            if (fileStatus.isDirectory()) {
    ...
    public boolean rename(String src, String dst) throws IOException {
        return getInstance().fs.rename(new Path(src), new Path(dst));
    }
